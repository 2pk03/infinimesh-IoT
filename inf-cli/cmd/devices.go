...
var mgmtDevIceStateMQTTCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		} else if token, _ := cmd.Flags().GetString("device-token"); token != "" {
			deviceID, _ := cmd.Flags().GetString("device-id")
			if deviceID == "" {
				return errors.New("device-id is required when using device-token auth")
			}
			if token == "" {
				return errors.New("device-token is required when using device-id auth")
			}
			if port == "" {
				port = "1883"
			}
		...
		if desired {
			...
			deviceID, _ := cmd.Flags().GetString("device-id")
			if deviceID == "" {
				return errors.New("device-id is required for desired state subscription")
			}
			topicTmpl, _ := cmd.Flags().GetString("topic")
			topic := fmt.Sprintf(topicTmpl, deviceID)
		...
		if report != "" {
			deviceID, _ := cmd.Flags().GetString("device-id")
			if deviceID == "" {
				return errors.New("device-id is required to publish report")
			}
			topicTmpl, _ := cmd.Flags().GetString("topic")
			topic := fmt.Sprintf(topicTmpl, deviceID)
			client.Publish(topic, 1, false, report)
...
func init() {
	...
	mgmtDevIceStateMQTTCmd.Flags().String("device-id", "", "Device ID for JWT-based MQTT auth (username)")
	mgmtDevIceStateMQTTCmd.Flags().String("device-token", "", "Device token for MQTT auth (password)")
	...
	mgmtDevIceStateMQTTCmd.Flags().String("topic", "shadow/devices/%s/desired", "Topic template; %s will be replaced with device ID")
